...
modelParams <- c("duration","peakTime","peak","peak2","peak3","trough","trough2",
...
myStudies <- lapply(dirs, makeStudies, spCodes=spWanted)
myClicks <- lapply(myStudies, getClickData)
filteredClicks <- lapply(modelNames, filterClicks, myClicks, detectorsWanted, modelParams)
bantClicks <- lapply(filteredClicks, makeBanterData)
